...
public class LogiqlDebugSolver implements InferenceSolver {
	...
	public Map<Integer, AnnotationMirror> solve(
		...
		for (Constraint constraint : constraints) {
			...
			output= output+ "+"+ constraint.getClass().getSimpleName() + "["+vStr[0]+","+vStr[1]+ "]=true.\n" ;
		}
		try {
		...
		//command[0]= "source /home/jianchu/Desktop/logicblox-x86_64-linux-4.1.8-91f645cf2a0e/etc/profile.d/logicblox.sh";
		...
		try {
			...
			AnnotationMirror OSuntrusted = AnnotationUtils.fromClass(elements, OsUntrusted.class);
			AnnotationMirror OStrusted = AnnotationUtils.fromClass(elements, OsTrusted.class);
			for (int i = 0; i < slots.size(); i++) {
				result.put(i, OSuntrusted);
			}
			
			Process p5 = Runtime.getRuntime().exec(command[5]);
			BufferedReader stdInput = new BufferedReader(new InputStreamReader(p5.getInputStream())); 
	...
	private String[] SlotsToStr(List<Slot> slots) {
		...
		for (int i =0; i< 2; i++) {
			...
			} else if (slots.get(i).getClass().equals(RefinementVariableSlot.class)){
				RefinementVariableSlot slot = (RefinementVariableSlot) slots.get(i);
				vStr[i] = Integer.toString( slot.getId());
